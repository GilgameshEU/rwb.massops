...
class RowNormalizer
{
     ...
     *
     * Логика приведения:
     * - Номер с «+» — возвращается как есть (международный формат).
     * - 11-значный номер начинающийся с «8» — заменяем «8» на «+7»
     *   (исторический российский формат 8-xxx-xxx-xx-xx).
     *   Замена актуальна только для RU; при другом коде страны Bitrix-парсер
     *   сам разберёт номер корректно через $defaultCountry.
     * - Все остальные номера отдаём парсеру без изменений —
     *   он добавит код страны из настройки phone_default_country.
     */
    private function preparePhone(string $phone): string
    {
        ...
        // Обработка российского «8-xxx» формата
